...
function cell_selector(id) {
    if (GLOBALS.isrunning == true) return;
...
function reset() {
    if (GLOBALS.isrunning == false) {
        ...
        alert(
            "Algorithm is running! Please Wait! If you want to halt the process please Refresh the page."
        );
}
...
function getadjacent_ids(id) {
    ...
    if ((id - 1) % 1000 > 0)
        if (
            document.getElementById(id - 1).classList.contains("disable") ==
            false
        )
            res.push(id - 1);
    if (id / 1000 - 1 > 1)
        if (
            document.getElementById(id - 1000).classList.contains("disable") ==
            false
        )
            res.push(id - 1000);
    if ((id + 1) % 1000 < n)
...
window.onresize = () => {
    if (window.innerWidth > 840) window.location = window.location.href;
};
...
if (GLOBALS.DEBUG_MODE == true) console.log(m, n);

for (let i = 1; i < m; i++) for (let j = 1; j < n; j++) create_cell(i, j);
    ...
    while (!queue.isEmpty()) {
        ...
        for (let j = 0; j < adj.length; j++) {
            if (visited.findIndex((ele) => ele == adj[j]) == -1) {
                visited.push(adj[j]);
                active_cell(adj[j]);
                if (GLOBALS.DEBUG_MODE == true) console.log("Visited:", adj[j]);
                await wait(GLOBALS.algo_speed);
                if (idtofind == adj[j]) return;
                inactive_cell(adj[j]);
                queue.push(adj[j]);
